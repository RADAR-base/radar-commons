...
class AvroDataMapperFactoryTest {
    private lateinit var factory: AvroDataMapperFactory
    @Before
    fun setUp() {
        factory = AvroDataMapperFactory()
    }
    ...
    fun mapRecord() {
        val actual = doMap(
            MEASUREMENT_KEY_SCHEMA, ObservationKey.getClassSchema(),
            "{\"userId\":\"u\", \"sourceId\": \"s\"}"
        )
        Assert.assertEquals("{\"projectId\":null,\"userId\":\"u\",\"sourceId\":\"s\"}", actual)
    }
    ...
    fun mapEnumLarger() {
        ...
        Assert.assertEquals("{\"e\":\"A\"}", actual)
    }
    ...
    fun mapEnumSmallerUnknown() {
        ...
        Assert.assertEquals("{\"e\":\"UNKNOWN\"}", actual)
    }
    ...
    fun mapAll() {
        val actual = doMap(
            ALL_TYPES_SCHEMA, ALL_TYPES_ALT_SCHEMA, "{" +
                    "\"e\":\"A\"," +
                    "\"i\":1," +
                    "\"l\":2," +
                    "\"d\":3.0," +
    ...
    private fun doMap(from: Schema, to: Schema, value: String): String {
        val mapper = factory.createMapper(from, to, null)
